...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen bg-gradient-to-br from-[#020617] via-[#0f172a] to-[#020617] text-white overflow-hidden">
      ...
      {/* CONTENT */}
      <section className="relative z-10 max-w-4xl mx-auto mt-10 px-6">
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              <h2 className="text-2xl mb-6">Válassz dátumot</h2>
              <div className="grid grid-cols-7 gap-3">
                {[...Array(30)].map((_, i) => (
                  <button key={i} onClick={()=>setSelectedDate(i+1)}
                    className={`p-3 rounded-xl ${selectedDate===i+1 ? 'bg-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.8)]' : 'bg-white/5 hover:bg-white/10'}`}>
                    {i+1}
                  </button>
                ))}
              </div>
              <button onClick={handleNext} className="mt-6 px-6 py-3 bg-purple-600 rounded-xl">Tovább</button>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              <h2 className="text-2xl mb-6">Válassz időpontot</h2>
              <div className="grid grid-cols-2 gap-4">
                {times.map(t => (
                  <button key={t} onClick={()=>setSelectedTime(t)}
                    className={`p-4 rounded-xl ${selectedTime===t ? 'bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.8)]' : 'bg-white/5 hover:bg-white/10'}`}>
                    {t}
                  </button>
                ))}
              </div>
              <button onClick={handleNext} className="mt-6 px-6 py-3 bg-blue-600 rounded-xl">Tovább</button>
            </motion.div>
          )}
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
              ...
              <div className="space-y-4">
                <input placeholder="Név" className="w-full p-4 bg-white/5 rounded-xl" />
                <input placeholder="Email" className="w-full p-4 bg-white/5 rounded-xl" />

                <button onClick={handleBooking}
                  className="w-full p-4 rounded-xl bg-gradient-to-r from-purple-500 to-blue-500 flex justify-center items-center">
