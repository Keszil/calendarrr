...
// Pricing szekció dinamikus betöltése (ez oldja meg a build hibát)
...
export default function Home() {
  ...
  const testimonials = [
    { name: "Kovács Anna", role: "Fodrász & Szalon tulajdonos", text: "Azóta, hogy használom az Időpont.app-ot, heti 6-8 órát spórolok az egyeztetéseken. Az ügyfelek imádják, hogy maguk tudnak időpontot foglalni.", rating: 5, avatar: "👩‍🦰" },
    { name: "Nagy Márk", role: "Életvezetési coach", text: "Sokkal professzionálisabb lett a szolgáltatásom. Kevesebb lemondás, több betelt időpont. Tavaly óta már a Pro csomagot használom.", rating: 5, avatar: "🧔" },
    { name: "Szabó Eszter", role: "Jogi tanácsadó", text: "A korábbi email-vissza-vissza írkálás helyett most tényleg csak a munkára kell koncentrálnom. Nagyon intuitív és megbízható rendszer.", rating: 5, avatar: "👩‍💼" },
    { name: "Tóth Péter", role: "Személyi edző", text: "Az ügyfeleim 90%-a magától foglal. Hihetetlen, mennyire leegyszerűsítette az életemet ez az eszköz.", rating: 5, avatar: "💪" }
  ]
  ...
  return (
    <main className="relative min-h-screen bg-zinc-950 dark:bg-zinc-950 text-white overflow-hidden transition-colors duration-300">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden">
        <motion.div animate={{ backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'] }} transition={{ duration: 40, repeat: Infinity }} className="absolute inset-0 bg-[radial-gradient(at_25%_25%,rgba(168,85,247,0.18),transparent_60%)]" />
        <motion.div animate={{ backgroundPosition: ['100% 30%', '0% 70%', '100% 30%'] }} transition={{ duration: 45, repeat: Infinity }} className="absolute inset-0 bg-[radial-gradient(at_75%_65%,rgba(34,211,238,0.18),transparent_65%)]" />
        <motion.div animate={{ x: [0, 220, -100, 0], y: [0, -150, 180, 0], scale: [1, 1.15, 0.9, 1] }} transition={{ duration: 32, repeat: Infinity }} className="absolute -top-40 -left-60 w-[800px] h-[800px] bg-purple-600/10 rounded-full blur-[120px]" />
        <motion.div animate={{ x: [0, -180, 150, 0], y: [0, 200, -120, 0], scale: [1.1, 0.85, 1.2, 1] }} transition={{ duration: 38, repeat: Infinity, delay: 8 }} className="absolute bottom-0 right-0 w-[900px] h-[900px] bg-cyan-500/10 rounded-full blur-[130px]" />
      </div>
      ...
      <header className="relative z-50 flex justify-between items-center p-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-6">
          <button onClick={() => setIsDark(!isDark)} className="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/20 transition flex items-center gap-2">
            {isDark ? '☀️ Világos' : '🌙 Sötét'}
          </button>
          <button className="px-6 py-2.5 rounded-2xl hover:bg-white/10 transition">Bejelentkezés</button>
          <button onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })} className="px-7 py-2.5 rounded-2xl bg-white text-black font-semibold hover:bg-white/90 transition">
            Ingyen kezdés
          </button>
        </div>
      </header>
      ...
      <section className="relative z-20 pt-20 pb-16 text-center max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
          <button onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })} className="px-10 py-4 text-lg font-semibold rounded-2xl bg-gradient-to-r from-purple-500 via-violet-500 to-cyan-500 hover:brightness-110 transition shadow-xl shadow-purple-500/30">
      ...
      <section className="relative z-20 max-w-6xl mx-auto mt-28 px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            <motion.div key={i} whileHover={{ y: -8 }} className="bg-zinc-900/70 border border-white/10 p-8 rounded-3xl hover:border-purple-500/40 transition-all">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="grid grid-cols-7 gap-3">
                  {[...Array(14)].map((_, i) => (
                    <button key={i} onClick={() => setSelectedDate(i)} className={`aspect-square rounded-2xl transition-all ${selectedDate === i ? 'bg-purple-600 scale-105' : 'bg-white/5 hover:bg-white/10'}`}>
                      {i + 1}
                    </button>
                  ))}
                </div>
            ...
            {step === 2 && (
              <motion.div key="2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {times.map(t => (
                    <button key={t} onClick={() => setSelectedTime(t)} className={`p-5 rounded-2xl text-lg transition-all ${selectedTime === t ? 'bg-cyan-500 text-black font-semibold' : 'bg-white/5 hover:bg-white/10'}`}>
                      {t}
                    </button>
                  ))}
                </div>
            ...
            {step === 3 && (
              <motion.div key="3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <input type="email" placeholder="Add meg az email címed" className="w-full p-5 bg-white/5 border border-white/10 rounded-2xl mb-6 focus:outline-none focus:border-purple-500" />
                <button onClick={handleBooking} disabled={loading} className="w-full py-5 bg-gradient-to-r from-purple-500 to-cyan-500 font-semibold rounded-2xl text-lg hover:brightness-110 disabled:opacity-70">
      ...
      <section className="relative z-20 max-w-6xl mx-auto mt-32 px-6">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {testimonials.map((t, i) => (
            <motion.div key={i} whileHover={{ scale: 1.02 }} className="bg-zinc-900/70 border border-white/10 p-8 rounded-3xl">
              ...
              <div className="mt-6 flex text-yellow-400">{'★'.repeat(t.rating)}</div>
            </motion.div>
          ))}
        </div>
      </section>

      {/* Pricing – dinamikusan betöltve */}
      <PricingSection />
      ...
      <section className="relative z-20 text-center py-24 border-t border-white/10 bg-black/40">
        ...
        <button onClick={() => alert('Regisztráció indítása...')} className="px-14 py-6 text-xl font-semibold rounded-2xl bg-gradient-to-r from-purple-600 to-cyan-500 hover:brightness-110 transition shadow-2xl">
