...
export default function Home() {
  const [demoStep, setDemoStep] = useState(1) // csak vizuális lépésváltás
  ...
  return (
    <main className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 relative">
      
      {/* Finom absztrakt háttér animáció */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        ...
        <motion.div
          ...
          transition={{ duration: 60, repeat: Infinity, delay: 10 }}
      ...
      {/* DEMO - Kisebb, GIF-szerű, nem kattintható */}
      <section className="py-20 bg-zinc-50 dark:bg-zinc-900 relative z-10">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-3xl shadow-xl overflow-hidden mx-auto max-w-2xl">
            <div className="p-10">
              {/* Hónap választó */}
              ...
              <div className="flex justify-center gap-3 mb-8">
                {[1, 2, 3].map((s) => (
                  <div
                    key={s}
                    className={`h-2 flex-1 max-w-[90px] rounded-full transition-all ${
              ...
              <AnimatePresence mode="wait">
                {demoStep === 1 && (
                  <motion.div key="1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
                    <p className="mb-6 text-zinc-500 font-medium">Válassz egy dátumot</p>
                ...
                {demoStep === 2 && (
                  <motion.div key="2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
                    <p className="mb-6 text-zinc-500 font-medium">Válassz időpontot</p>
                    <div className="grid grid-cols-2 gap-3">
                      {times.map(t => (
                        <div key={t} className="py-4 rounded-2xl bg-zinc-100 dark:bg-zinc-700 text-sm font-medium">
                          {t}
                        </div>
                      ))}
                    </div>
                ...
                {demoStep === 3 && (
                  <motion.div key="3" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
                    <p className="mb-6 text-zinc-500 font-medium">Add meg az adataidat</p>
                    <div className="w-full h-14 bg-zinc-100 dark:bg-zinc-700 rounded-2xl mb-6"></div>
                    <div className="w-full py-4 bg-emerald-600 text-white font-medium rounded-2xl">Foglalom ezt az időpontot</div>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
          ...
          {/* Demo lépésváltó (csak vizuális) */}
          <div className="flex justify-center gap-3 mt-8">
            {[1,2,3].map(s => (
              <button
                key={s}
                onClick={() => setDemoStep(s)}
                className={`w-3 h-3 rounded-full transition-all ${demoStep === s ? 'bg-emerald-600 scale-125' : 'bg-zinc-300 dark:bg-zinc-600'}`}
              />
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {pricingPlans.map((plan, index) => (
              <motion.div
                ...
                className={`relative rounded-3xl p-8 flex flex-col border bg-white dark:bg-zinc-800 transition-all ${plan.popular ? 'border-emerald-600 shadow-xl scale-[1.02]' : 'border-zinc-200 dark:border-zinc-700'}`}
              >
                {plan.popular && <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-emerald-600 text-white text-xs font-bold px-6 py-1.5 rounded-full">LEGNÉPSZERŰBB</div>}
                ...
                <button onClick={() => alert(`${plan.name} csomag kiválasztva (demo)`)} className={`w-full py-4 rounded-2xl font-medium transition-all ${plan.popular ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : 'border border-zinc-300 dark:border-zinc-600 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}>
                  {plan.cta}
                </button>
              </motion.div>
            ))}
      ...
      <section className="py-24 bg-emerald-600 text-white relative z-10">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <button onClick={() => alert('Regisztráció indítása...')} className="px-14 py-6 text-xl font-semibold bg-white text-emerald-700 rounded-full hover:bg-emerald-50 transition shadow-lg">
